...
def process_and_ocr():
    ...
    crop_line1 = img.crop((0, int(h * 0.62), w, int(h * 0.82))).resize((128, 32))
    crop_line2 = img.crop((0, int(h * 0.82), w, h)).resize((128, 32))
    ...
    with torch.no_grad():
        logits1 = model(t1)
        logits2 = model(t2)
        text1 = model.decode_word(logits1)
        text2 = model.decode_word(logits2)

    print("\n============================================================")
    print("🔍 RECONOCIMIENTO OCR DE PALABRAS COMPLETAS EN PYTHON")
    print("============================================================")
